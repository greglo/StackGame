...
public class BlockUI extends JPanel {
    ...
    public static Dimension getRealBlockSize(Dimension viewSize) {
        return new Dimension(viewSize.width * CELLWIDTH, viewSize.height
                * CELLHEIGHT);
    }
    ...
    protected class GeneralListener implements StackMachineListener,
            SelectionManagerListener, BlockManagerListener {
    ...
    public void displayError(String text) {
        JOptionPane.showMessageDialog(blockui, text, "Error",
                JOptionPane.ERROR_MESSAGE);
    }

    /** Forwards input events to the responsible handlers */
    protected class BlockEventForwarder implements MouseListener,
    ...
    protected void updateCurrentStackMachine() {
        ...
        if (currentStackMachine != null) {
            int stackMachineSize = currentStackMachine.getInstructions().size() + 1; // +1
                                                                                     ...
                                                                                     // 1
                                                                                     // command
            setPreferredSize(new Dimension(CELLWIDTH, stackMachineSize
                    * CELLHEIGHT));
        } else
    ...
    public void paintComponent(Graphics g) {
        ...
        if (currentStackMachine != null) {
            ...
            Dimension viewSize = new Dimension(2 * CELLWIDTH, CELLHEIGHT
                    * height);
